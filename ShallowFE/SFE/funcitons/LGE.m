...
%                     DPrime = DPrime + options.ReguAlpha*trace(DPrime)*eye(size(DPrime));
...
if bEigs
    ...
    if bPCA && ~bD
        ...
        if bChol
            option.cholB = 1;
            [eigvector, eigvalue] = eigs(WPrime,R,ReducedDim,0,option);
        else
            [eigvector, eigvalue] = eigs(WPrime,DPrime,ReducedDim,0,option);
        end
    end
